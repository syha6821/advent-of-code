...
def solve(filepath):
    scanners = list(read_scanners(filepath))
    inspected = dict()
    ...
    while len(inspected) != len(scanners):
        for i,j in ((i,j) for i,j in combinations(range(0,len(scanners)),2) if not i == j):
            ...
            print("I2",i2,i2_face,tuple(p1-p2 for p1,p2 in zip(i1,b1)),inspected[i]["face"],tuple(p1-p2 for p1,p2 in zip(i2,b2)))
            ...
            print(i,j,"len",len(set(adjusted_beacons).difference(found)),inspected_location)
    ...
    # print("result")
    # for beacon in sorted(found,key= lambda b:b[0]):
    #     print(*beacon)
    return len(found)
