...
char Unit::Look(Direction direction)
{
    return (char)map_.at(PositionAt(direction)).material();
}
...
bool Unit::Move(Direction& direction)
{
    ...
    {
        Direction before_direction = direction;
        Position destination = map_.LinkedPosition(location_, direction);
        if(map_.at(destination).material() == Material::Rock)
        {
