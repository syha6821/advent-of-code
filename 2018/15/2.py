...
def print_area(area):
    for line in area:
        print(*[tile if tile in [EMPTY,WALL] else tile.type for tile in line],sep="")

def calc(area,goblins,elves,power):
    ...
    while goblins > 0:
        ...
        for unit in units:
            ...
            if area[unit.row][unit.col] == unit:
                unit.move(area)
                dead = unit.attack(area,3 if unit.type == GOBLIN else power)
                next_units.append(unit)
                if dead:
                    if dead.type == GOBLIN:
                        goblins -= 1
                        ...
                        return False
        units = sorted([unit for unit in next_units if area[unit.row][unit.col] != EMPTY],key = lambda u : (u.row,u.col))
        if finished:
            break
        cnt += 1
    return cnt * sum(unit.hp for unit in units)

def copy_units(units):
    return [Unit(unit.type,unit.row,unit.col) for unit in units]

def sol(file,p):
    area,goblins,elves = read_input(file)
    return calc(area,goblins,elves,p)
