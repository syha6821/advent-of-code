...
def simulate(immunes,infections):
    immunes = deepcopy(immunes)
    infections = deepcopy(infections)
    ...
    return (IMMUNE if immunes else INFECTION),sum([unit.count for unit in (immunes if immunes else infections)])
...
def solve(file):
    ...
    while l <= r:
        mid = (l + r) // 2
        boost(immunes,mid)
        winner,result = simulate(immunes,infections)
        boost(immunes,-mid)
        if winner == IMMUNE:
            print("!")
            if mid < ans[1]:
